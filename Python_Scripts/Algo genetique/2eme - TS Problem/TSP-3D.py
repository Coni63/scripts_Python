...
class Population:
    def __init__(self, algo_setting, pb_setting):
        ...
        for i in range(self.number_of_cities):
            ...
            d = algo_setting["width"]
            b = algo_setting["border"]
            self.pts_list.append((random.randrange(b, w-b ), random.randrange(b, h-b), 0))
...
class MyWindow(pyglet.window.Window):
    ...
    def project(self):
        ...
        pyglet.gl.glOrtho(0, 1200, 0, 1000, -1, 1)
    ...
    #     pyglet.gl.glMatrixMode(pyglet.gl.GL_PROJECTION)
    #     pyglet.gl.glLoadIdentity()
    #     pyglet.gl.gluPerspective(65, width / float(height), .1, 1000)
    #     pyglet.gl.glMatrixMode(pyglet.gl.GL_MODELVIEW)
    ...
    def on_draw(self):
        ...
        for city in pop.pts_list:
            pyglet.gl.glVertex3i(*city)
        pyglet.gl.glEnd()

        pyglet.gl.glBegin(pyglet.gl.GL_LINE_STRIP)
        for path in self.best_ind.path:
            pyglet.gl.glVertex3i(*path)
        pyglet.gl.glEnd()
