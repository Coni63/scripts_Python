...
class Strategy:
            ...
            if each_bases.robot_available > 1:
                for each_other_bases in ordered_bases:
                    if each_other_bases != each_bases: 
                        if each_other_bases.balance < 2:
                            support = min( each_bases.robot_available, 2-each_other_bases.balance)
    ...
    def set_availability(self):
        for each in Factory.instances.values():
            each.robot_available = max(0, min(each.defense, each.defense+each.ongoing_support-each.ongoing_ennemies))
            each.balance = each.defense+each.ongoing_support-each.ongoing_ennemies
    ...
    def expand_fast(self):
        ...
        for factory in Factory.my_bases.values():
            list_of_voisin = [Factory.instances[a] for a in G.optimised_voisin[factory.ID]]
            list_of_voisin.sort(key = lambda x : G.distances[(factory.ID, x.ID)])
            queue = []
            safety_base_vide = 1
            safety_base_ennemie = 5
            for each_voisin in list_of_voisin:
                if each_voisin.prod >= prod_mini:
                    opti_path = G.optimised_path[(factory.ID,each_voisin.ID)]
                    if each_voisin.team == 0:
                        qte = each_voisin.defense + each_voisin.ongoing_ennemies
                        if factory.robot_available >= qte + safety_base_vide:
                            queue.append((factory.ID, opti_path, qte + safety_base_vide))
                        ...
                        if factory.robot_available >= qte + safety_base_ennemie:
                            queue.append((factory.ID, opti_path, qte + safety_base_ennemie))
    ...
    def create_strategy(self):
        ...
        for each_base in Factory.my_bases.values():
            if each_base.risk[0] == 1 or each_base.risk[1] == 1:
                ...
                self.command.append("MOVE %s %s %s" %(each_base.ID, target, qte))
                
        print(Cyborg.incoming_attacks, file=sys.stderr)
...
print(G.optimised_voisin, file=sys.stderr)
